...
import com.example.e_auction.databinding.ActivityUserBidsBinding
...
import com.example.e_auction.adapter.UserAddedBidsAdapter
...
class UserBidsActivity : AppCompatActivity(), Callback<Response> {
    ...
    lateinit var userAddedBidsAdapter: UserAddedBidsAdapter
    lateinit var dataSource:MutableList<Bid>
    private lateinit var context: Context

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding=DataBindingUtil.setContentView(this,R.layout.activity_user_bids)
        ...
        binding.imgAdd.setOnClickListener{
            Intent(this,AddOrUpdateBidActivity::class.java).apply{
                startActivity(this)
            }
        }
        binding.rvUserBids.layoutManager = LinearLayoutManager(this)
    ...
    private fun showUserBids(){
        context = this
        dataSource = DataProvider.response.userBids
        if(dataSource.size>0){
            userAddedBidsAdapter = UserAddedBidsAdapter(this,context,dataSource)
            binding.rvUserBids.adapter = userAddedBidsAdapter
